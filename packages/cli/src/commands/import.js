...
function parseTaskLine(line, lineNumber) {
  ...
  // Extract priority [P0-P3]
  const priorityMatch = taskLine.match(/\[(P[0-3])\]/);
  if (priorityMatch) {
    task.priority = priorityMatch[1];
  }

  // Extract tags [tag1,tag2]
  const tagsMatch = taskLine.match(/\[([a-zA-Z0-9_,\s]+)\](?!\s*\[P[0-3]\]|\s*\d+h)/);
  if (tagsMatch) {
    task.tags = tagsMatch[1].split(',').map((t) => t.trim()).filter(Boolean);
  }

  // Extract hours [4h] or [0.5h]
  const hoursMatch = taskLine.match(/\[(\d+(?:\.\d+)?)h\]/i);
  if (hoursMatch) {
    task.estimatedHours = parseFloat(hoursMatch[1]);
  }

  // Extract assignee [@name]
  const assigneeMatch = taskLine.match(/\[@([^\]]+)\]/);
  if (assigneeMatch) {
    task.assignee = assigneeMatch[1];
  }

  // Extract dependencies [依赖:id] or [dependency:id]
  const depMatch = taskLine.match(/\[(?:依赖|dependency):([^\]]+)\]/i);
  if (depMatch) {
    const depString = depMatch[1];
    // Could be comma-separated IDs or numbers
...
function resolveDependencies(tasks, createdTasks) {
  createdTasks.forEach((task, index) => {
    ...
    if (originalTask.dependencies && originalTask.dependencies.length > 0) {
      const resolvedDeps = originalTask.dependencies.map((dep) => {
        // If dependency is a number, resolve to task ID
        const num = parseInt(dep);
        if (!isNaN(num)) {
          // Assuming 1-based index from user input
  ...
  try {
    ...
    if (options.dryRun) {
      ...
      tasks.forEach((task, index) => {
        ...
        console.log(chalk.dim(`   Priority: ${task.priority} | Tags: ${task.tags.join(', ') || 'none'} | Hours: ${task.estimatedHours}`));
      });
    ...
    console.log(chalk.gray('  seshflow list     - View all tasks (if available)'));
  } catch (error) {

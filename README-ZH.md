...
- 通过 `seshflow issue` 从任意目录把问题轻量反馈回当前正在使用的 Seshflow workspace
...
- `seshflow init`
  ...
  - 同时把当前 workspace 记为后续 `seshflow issue` 的默认反馈目标
- `seshflow ncfr`
...
- `seshflow issue`
  - 用一次命令把问题落回已记住的 Seshflow workspace
  - 必填字段是 `trigger`、`actual`、`expected`、`impact`
  - 会自动附带来源目录、来源 workspace 状态和来源分支（如果能解析）
- `seshflow next`
